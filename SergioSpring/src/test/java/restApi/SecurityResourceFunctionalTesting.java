...
public class SecurityResourceFunctionalTesting {
    ...
    private static final String url = "http://localhost:8080/SergioSpring.0.0.1-SNAPSHOT/api/v0";
    ...
    public void testPlayerOK() {
        String response = new RestBuilder<String>(url).path(Uris.SECURITY).path(Uris.SECURITY_ANNOTATION).basicAuth("u1", "123456")
                .clazz(String.class).get().build();
        System.out.println("INFO >>>>> " + response);
    }
    ...
    //Prueba de player
    @Test
    public void testPlayerOK2() {
        String response = new RestBuilder<String>(url).path(Uris.SECURITY).path(Uris.SECURITY_URI).basicAuth("u1", "123456")
                .clazz(String.class).get().build();
        System.out.println("INFO >>>>> " + response);
    }
    ...
    @Test
    public void testPlayerUnauthorizedUser() {
        try {
        	 String response = new RestBuilder<String>(url).path(Uris.SECURITY).path(Uris.SECURITY_ANNOTATION).basicAuth("user", "123456")
                     .clazz(String.class).get().build();
            System.out.println("INFO >>>>> " + response);
            fail();
    ...
    @Test
    public void testPlayer2UnauthorizedUser() {
        try {
        	String response = new RestBuilder<String>(url).path(Uris.SECURITY).path(Uris.SECURITY_URI).basicAuth("user", "123456")
                    .clazz(String.class).get().build();
            System.out.println("INFO >>>>> " + response);
            fail();
        } catch (HttpClientErrorException httpError) {
